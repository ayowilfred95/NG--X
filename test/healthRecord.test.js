  ...
  describe("RegisterNewDoctor", function () {
    ...
    describe("addCaseSummary", function () {
      it("allows approved doctors to add CaseSummary to the patient records", async () => {
        ...
        //have doctor call on function that requires approval -- expect error revert
        await expect(
          myContract_d.addCaseSummary("Migraines", 1)
        ).to.be.revertedWith(
          "Doctor does not have approval to treat this patient!"
        );
        //have patient approve doctor
        ...
        await myContract_d.connect(doctor_);
        const tx_2 = await myContract_d.addCaseSummary("Type II Diabetes", 1);
        await tx_2.wait();
        const tx_3 = await myContract_d.addCaseSummary("Asthma", 1);
        await tx_3.wait();
        const results = await myContract_d.viewPatientRecords(1);
        ...
        assert.equal(results_2.caseSummaries[0], "Type II Diabetes");
        assert.equal(results_2.caseSummaries[1], "Asthma");
      });
    });
